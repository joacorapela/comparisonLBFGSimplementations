...
def main(argv):
    ...
    parser.add_argument("--resultsFilenamePattern", help="results filename pattern", default="../results/scipy_manualgrad_{:s}.csv")
    ...
    x0 = np.array([float(str) for str in paramsConfig["init_params"]["x0"][1:-1].split(",")])
    ...
    if generativeFunc=="rosenbrock":
        evalFunc = testFunctions.rosenbrockWithManualgrad
    elif generativeFunc=="sixHumpCamel":
        evalFunc = testFunctions.sixHumpCamelWithManualgrad
    elif generativeFunc=="zakharov":
        evalFunc = testFunctions.zakharovWithManualgrad
    else:
    ...
    for i in range(nRepeats):
        startTime = time.time()
        optimRes = scipy.optimize.minimize(fun=evalFunc, x0=x0, method='L-BFGS-B', jac=True, options=minimizeOptions)
